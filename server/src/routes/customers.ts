...
    // Create a new array for the main query to avoid parameter mismatch

    const mainQueryParams = [...queryParams, limit, offset];



    const [customers] = await pool.execute(query, mainQueryParams);
...
    const [customers] = await pool.execute(query, [id]);
...
    const [codeResult] = await pool.execute(codeQuery);
...
      const [emailResult] = await pool.execute(emailQuery, [email]);
...
      const [emailResult] = await pool.execute(emailQuery, [email, id]);
...
    const [ordersResult] = await pool.execute(ordersQuery, [id]);

   

    const reservationsQuery = 'SELECT COUNT(*) as count FROM reservations WHERE customer_id = ?';

    const [reservationsResult] = await pool.execute(reservationsQuery, [id]);
...
    const [activeResult] = await pool.execute(activeQuery);
...
    const [thisMonthResult] = await pool.execute(thisMonthQuery);
...
    const [withOrdersResult] = await pool.execute(withOrdersQuery);
