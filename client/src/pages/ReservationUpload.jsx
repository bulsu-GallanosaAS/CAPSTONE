...
export default function ReservationUpload() {
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        try {
          ...
          const customerResponse = await fetch(`http://localhost:5000/api/customers?email=${encodeURIComponent(reservationData.email)}&limit=1`, {
            headers: {
          ...
          if (!customerId) {
            ...
            const createCustomerResponse = await fetch('http://localhost:5000/api/customers', {
              method: 'POST',
              headers: {
            ...
            if (createCustomerResponse.ok && customerResult.success) {
              ...
              if (customerResult.message?.includes('Email already exists')) {
                const retryResponse = await fetch(`http://localhost:5000/api/customers?email=${encodeURIComponent(reservationData.email)}&limit=1`, {
                  headers: {
                    'Authorization': `Bearer ${token}`
                  }
                });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Update URL:', `http://localhost:5000/api/reservations/${reservationId}`);
